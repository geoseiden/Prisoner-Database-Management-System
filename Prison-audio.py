from playsound import playsound         #pip install required
...
mydb=my.connect(host='localhost',user='root',password='root',database='school')
cur=mydb.cursor()
#playsound(r'Welcome.wav')
#time.sleep(2)
#playsound(r'Choices.wav')
...
def menu():
    ...
    mydb=my.connect(host='localhost',user='root',passwd='root',database='school')
    cur=mydb.cursor()
    cur.execute('select *  from prison;')
    r=cur.fetchall()
    global root1
    root1 = tk.Tk()
    root1.title('Prison Management')
    ...
    root2.grid(row=1,column=0,columnspan=5,sticky='nsew')
    ...
    tv.tag_configure('M',font='Arial 10')
    scrollbar=Scrollbar(root2,orient='vertical',command=tv.yview)
    scrollbar.pack(side=RIGHT,fill='y')
    
    frm=Frame(root1)
    frm.grid(row=2,column=0,columnspan=5)
    add_b = tk.Button(master=frm,text='New Prisoner',command=lambda:[root1.destroy(),add()],height=2,width=15,font=('Arial Black',12),borderwidth=0,bg='#696969',fg='black')
    add_b.grid(row=0,column=0,sticky='nsew')
    coh(add_b,'#8c8c8c','#696969')
    update_b = tk.Button(master=frm,text='Update Prisoner Data',command=lambda:[root1.destroy(),update()],height=2,width=20,font=('Baskerville Old Face',12),borderwidth=0,bg='#696969',fg='black')
    update_b.grid(row=0,column=1,sticky='nsew')
    coh(update_b,'#8c8c8c','#696969')
    delete_b = tk.Button(master=frm,text='Delete Prisoner Data',command=lambda:[root1.destroy(),delete()],height=2,width=20,font=('Baskerville Old Face',12),borderwidth=0,bg='#696969',fg='black')
    delete_b.grid(row=0,column=2,sticky='nsew')
    coh(delete_b,'#8c8c8c','#696969')
    exit_b = tk.Button(master=frm,text='Exit',command=lambda:[bye(),root1.destroy()],height=2,width=10,font=('Baskerville Old Face',12),borderwidth=0,bg='tomato',fg='black')
    ...
    mmenu.add_cascade(label='About',menu=crmenu)
    crmenu.add_command(label='Credits',command=lambda:[os.startfile("https://github.com/geoseiden")])
...
def add():
    ...
    def submit_add():
        ...
        if check_exists(prno_entry)==False:
            ...
            playsound(r'check exists-no.wav')
            root.destroy()
            time.sleep(2)
            add()
    root = tk.Tk()
    root.title('New Prisoner')
    ...
    exit_b = tk.Button(master=root,text='Back',command=lambda:[root.destroy(),menu()],width=10,font=('Arial',10),borderwidth=0,bg='tomato',fg='white')
...
def delete():
    ...
    def submit_delete():
        ...
        status['text'] = text
    root = tk.Tk()
    root.title('Delete Prisoner Data')
    ...
    prno = tk.Entry(master=root)
    prno.grid(row=0,column=1)
    exit_b = tk.Button(master=root,text='Back',command=lambda:[root.destroy(),menu()],font=('Arial',10),borderwidth=0,bg='tomato',fg='white')
...
def update():
    ...
    root.resizable(False,False)
    updatename_button = tk.Button(master=root,text='Update Name',command=lambda:[updatename(),root.destroy()],borderwidth=0,bg='#696969',fg='white')
    updatename_button.grid(row=0,column=0,sticky='nsew')
    coh(updatename_button,'#8c8c8c','#696969')
    updatecity_button = tk.Button(master=root,text='Update City',command=lambda:[updatecity(),root.destroy()],borderwidth=0,bg='#696969',fg='white')
    updatecity_button.grid(row=1,column=0,sticky='nsew')
    coh(updatecity_button,'#8c8c8c','#696969')
    updategender_button = tk.Button(master=root,text='Update Gender',command=lambda:[updategender(),root.destroy()],borderwidth=0,bg='#696969',fg='white')
